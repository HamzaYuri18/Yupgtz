...
export const saveContractToRapport = async (contractData: ContractData): Promise<boolean> => {
  try {
    ...
    const { data: existing } = await supabase
      .from('rapport')
      .select('id')
      .eq('numero_contrat', insertData.numero_contrat)
      .eq('type', insertData.type)
      .eq('date_operation', insertData.date_operation)
      .maybeSingle();

    if (existing) {
      console.log('⚠️ Contrat déjà existant dans rapport, pas d\'insertion');
      return true;
    }
...
export const checkAffaireInRapport = async (numeroContrat: string, datePaiement: string): Promise<any | null> => {
  try {
    ...
    const { data, error } = await supabase
      ...
      .eq('numero_contrat', numeroContrat)
      .eq('type', 'Affaire')
      .eq('date_operation', datePaiement)
      .maybeSingle();
...
export const saveAffaireContract = async (contractData: ContractData): Promise<boolean> => {
  try {
    ...
    // Vérifier les doublons dans rapport AVANT d'insérer
    const sessionDate = getSessionDate();
    ...
    if (existingRapport) {
      console.log('⚠️ Contrat Affaire existe déjà dans rapport, pas d\'insertion');
    } else {
...
export const saveTermeContract = async (
  ...
  try {
    ...
    if (existingRapport) {
      console.log('⚠️ Contrat Terme existe déjà dans rapport, pas d\'insertion');
    } else {
...
// Fonction pour supprimer un crédit directement de liste_credits
export const deleteCredit = async (id: number): Promise<boolean> => {
  try {
    const { error } = await supabase
      .from('liste_credits')
      .delete()
...
const resetAttestationStatutInCarnet = async (numeroAttestation: string | null | undefined) => {
  ...
  try {
    ...
    console.error('Erreur remise attestation à null:', err);
  }
};

const saveSuppressionRecord = async (
  rapportRow: Record<string, unknown>,
  motif: string,
  suppressPar: string,
  ...
  await supabase.from('reporting_suppression').insert({
    rapport_id: rapportRow.id as number,
    type: rapportRow.type as string,
    branche: rapportRow.branche as string ?? null,
    numero_contrat: rapportRow.numero_contrat as string ?? null,
    prime: rapportRow.prime as number ?? 0,
    assure: rapportRow.assure as string ?? null,
    mode_paiement: rapportRow.mode_paiement as string ?? null,
    type_paiement: rapportRow.type_paiement as string ?? null,
    montant_credit: rapportRow.montant_credit as number ?? null,
    montant: rapportRow.montant as number ?? 0,
    echeance: rapportRow.echeance ? String(rapportRow.echeance) : null,
    date_paiement_prevue: rapportRow.date_paiement_prevue ? String(rapportRow.date_paiement_prevue) : null,
    cree_par: rapportRow.cree_par as string ?? null,
    created_at_original: rapportRow.created_at as string ?? null,
    motif_suppression: motif,
    supprime_par: suppressPar,
    supprime_le: new Date().toISOString(),
    numero_attestation: numeroAttestation ?? (rapportRow.numatt as string) ?? null,
...
export const deleteRapportContract = async (
  ...
  try {
    ...
    if (fetchError) { console.error('Erreur fetch rapport:', fetchError.message); return false; }
    if (!contract) { console.error('Rapport introuvable id:', id); return false; }

    await saveSuppressionRecord(contract, motif, suppressPar, contract.numatt ?? null);

    let numeroAttestation: string | null = contract.numatt ?? null;
    ...
    if (contract.type === 'Terme') {
      const { data: termeRow } = await supabase
        ...
        .eq('echeance', contract.echeance)
        .maybeSingle();

      if (termeRow?.numatt) numeroAttestation = String(termeRow.numatt);

      await supabase.from('terme').delete()
        .eq('numero_contrat', contract.numero_contrat)
        .eq('echeance', contract.echeance);
      ...
      if (affaireRow?.numatt) numeroAttestation = String(affaireRow.numatt);

      await supabase.from('affaire').delete().eq('numero_contrat', contract.numero_contrat);
    } else if (contract.type === 'Avenant') {
      await supabase.from('Avenant_Changement_véhicule').delete().eq('numero_contrat', contract.numero_contrat);
    } else if (contract.type === 'Encaissement pour autre code') {
      await supabase.from('encaissement_autre_code').delete().eq('numero_contrat', contract.numero_contrat);
    }
    ...
    if (rapportError) { console.error('Erreur delete rapport:', rapportError.message); return false; }

    await supabase.from('liste_credits').delete().eq('numero_contrat', contract.numero_contrat);
    await supabase.from('Cheques').delete().eq('Numero_Contrat', contract.numero_contrat);
    await supabase.from('suivie_realisations').delete().eq('numero_contrat', contract.numero_contrat);
...
export const deleteAffaireContract = async (id: number, motif: string, suppressPar: string): Promise<boolean> => {
  try {
    const { data: contract, error: fetchError } = await supabase
      .from('affaire')
      .select('*')
    ...
    if (fetchError) { console.error('Erreur fetch affaire:', fetchError.message); return false; }
    if (!contract) { console.error('Affaire introuvable id:', id); return false; }

    const numeroAttestation: string | null = contract.numatt ? String(contract.numatt) : null;
    ...
    if (rapportRows && rapportRows.length > 0) {
      await saveSuppressionRecord(rapportRows[0], motif, suppressPar, numeroAttestation);
    }

    const { error: affaireError } = await supabase.from('affaire').delete().eq('id', id);
    if (affaireError) { console.error('Erreur delete affaire:', affaireError.message); return false; }

    await supabase.from('rapport').delete().eq('numero_contrat', contract.numero_contrat).eq('type', 'Affaire');
...
export const deleteTermeContract = async (id: number, motif: string, suppressPar: string): Promise<boolean> => {
  try {
    const { data: contract, error: fetchError } = await supabase
      .from('terme')
      .select('*')
    ...
    if (fetchError) { console.error('Erreur fetch terme:', fetchError.message); return false; }
    if (!contract) { console.error('Terme introuvable id:', id); return false; }
    ...
    const { error: termeError } = await supabase.from('terme').delete().eq('id', id);
    if (termeError) { console.error('Erreur delete terme:', termeError.message); return false; }

    await supabase.from('rapport').delete()
      .eq('numero_contrat', contract.numero_contrat)
      .eq('type', 'Terme')
      .eq('echeance', contract.echeance);
...
const EXCLUDED_REMARQUES = ['rt', 'payé par note de credit', 'vendu'];

const isExcludedRemarque = (remarque: string | null | undefined): boolean => {
  if (!remarque) return false;
...
export const getTotalTermesByMonth = async (monthName: string, year: string): Promise<number> => {
  try {
    const tableName = `table_terme_${monthName}_${year}`;
    ...
    if (error) {
      console.error(`❌ Erreur lors du comptage total des termes:`, error);
      return 0;
    }
    return count || 0;
  } catch (error) {
    console.error('❌ Erreur générale getTotalTermesByMonth:', error);
    return 0;
  }
};
...
// Prefixes de contrats suivis pour la productivité
const SUIVI_PREFIXES = ['606', '604', 'CI0805', 'CI0210'];
const SUIVI_USERS = ['ahlem', 'rouae'];
const SUIVI_START_DATE = '2026-06-18';

export const saveSuivieRealisation = async (contractData: ContractData): Promise<void> => {
  try {
    const user = (contractData.createdBy || '').toLowerCase();
    if (!SUIVI_USERS.includes(user)) return;

    const contractNumber = (contractData.contractNumber || '').trim().toUpperCase();
    const matchedPrefix = SUIVI_PREFIXES.find(p => contractNumber.startsWith(p));
    if (!matchedPrefix) return;

    const today = new Date().toISOString().split('T')[0];
    if (today < SUIVI_START_DATE) return;

    const primeNette = Number(contractData.premiumAmount);
    if (isNaN(primeNette) || primeNette <= 0) return;

    let typeContrat = '';
    if (matchedPrefix === '606') typeContrat = 'Santé Internationale';
    else if (matchedPrefix === '604') typeContrat = 'Santé Nationale';
    else if (matchedPrefix === 'CI0805') typeContrat = 'Habitation';
    else if (matchedPrefix === 'CI0210') typeContrat = 'Transport Marchandise';
    ...
    const { error } = await supabase
      ...
      .insert([{
        ...
        branche: contractData.branch || '',
        assure: contractData.insuredName || '',
        prime_ttc: primeNette,
        prime_nette: primeNette,
        utilisateur: contractData.createdBy || '',
        date_realisation: today
      }]);

    if (error) {
      console.error('❌ Erreur saveSuivieRealisation:', error);
    }
  } catch (err) {
    console.error('❌ Erreur saveSuivieRealisation:', err);
  }
};
...
export interface RemarqueMonthStats {
  ...
  vendu: number;
  rt: number;
  payeParNoteCredit: number;
  total: number;
}

export const getRemarqueStatsByMonth = async (months: string[]): Promise<RemarqueMonthStats[]> => {
  const results: RemarqueMonthStats[] = [];
  ...
  for (const month of months) {
    ...
    const year = parts[1];
    const normalizedMonth = monthsFR[rawMonth] || rawMonth;
    ...
    try {
      ...
      if (error || !data) {
        results.push({ month, vendu: 0, rt: 0, payeParNoteCredit: 0, total: 0 });
        continue;
      }

      let vendu = 0, rt = 0, payeParNoteCredit = 0;
      for (const row of data) {
        const r = (row.remarque || '').toLowerCase().trim();
        if (r === 'vendu') vendu++;
        else if (r === 'rt') rt++;
        else if (r === 'payé par note de credit' || r === 'paye par note de credit') payeParNoteCredit++;
      }

      results.push({ month, vendu, rt, payeParNoteCredit, total: vendu + rt + payeParNoteCredit });
    } catch {
      results.push({ month, vendu: 0, rt: 0, payeParNoteCredit: 0, total: 0 });
    }
  }
...
export const getRemarqueContractsByMonth = async (month: string, remarqueType: string): Promise<any[]> => {
  const monthsFR: Record<string, string> = {
    'janvier': 'janvier', 'février': 'fevrier', 'fevrier': 'fevrier',
    'mars': 'mars', 'avril': 'avril', 'mai': 'mai', 'juin': 'juin',
    'juillet': 'juillet', 'août': 'aout', 'aout': 'aout',
  ...
  try {
    const { data, error } = await supabase
      .from(tableName)
      .select('numero_contrat, assure, echeance, prime, num_tel, num_tel_2, statut, remarque')
      .ilike('remarque', remarqueType);

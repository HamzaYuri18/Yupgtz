...
export default function MissingAttestationModal({
  ...
  const uploadScanToSupabase = async (file: File, numeroAttestation: string): Promise<string> => {
    ...
    if (error) throw new Error(`Échec upload scan: ${error.message}`);
    ...
    try {
      for (const attestation of attestations) {
        ...
        if (attestation.motif === 'Annulé' && attestation.scanFile) {
          scanUrl = await uploadScanToSupabase(attestation.scanFile, attestation.numero);
        }

...
import { authenticateUser, saveSession, getSession, isUserLockedToday } from '../utils/auth';
...
const LoginForm: React.FC<LoginFormProps> = ({ onLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Vérifier s'il y a une session active d'un autre utilisateur
    ...
    // Vérifier si l'utilisateur est bloqué aujourd'hui (Ahlem / Rouae)
    const locked = await isUserLockedToday(username);
    if (locked) {
      setError('Vous avez déjà terminé votre session aujourd\'hui. Vous pourrez vous reconnecter demain.');
      setIsLoading(false);
      return;
    }
